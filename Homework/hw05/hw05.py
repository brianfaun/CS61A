...
class VendingMachine:
    ...
    """
    "*** YOUR CODE HERE ***"
    def __init__(self, item, cost, amount=0, balance=0):
        self.item = item
        self.cost = cost
        self.amount = amount
...
def generate_paths(t, x):
    ...
    for branch in t.branches:
        for b in generate_paths(branch, x):
            #if b.is_leaf and b.label == x:
            yield [t.label] + b
...
def deep_map(f, link):
    ...
    if type(link) == Link:
        ...
        for link in link.rest:
    else:
        return link

    if link.rest is Link.empty: #this is "leaf"
        link.first = f(link.first) 
    """
    if type(link) == Link:
        link.first = f(link.first)
