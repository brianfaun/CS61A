...
def closer_city(lat, lon, city1, city2):
    ...
    if distance(city1, target)> distance(city2, target):
...
def acorn_finder(t):
    ...
    if label(t) == 'acorn':
        return True
    for elem in branches(t):
        if acorn_finder(elem):
            return True
    return False
...
def sprout_leaves(t, vals):
    ...
    if is_leaf(t):
        sprout_list = []
        for x in vals:
            sprout_list.append(tree(x))
        return tree(label(t),sprout_list)
        #return tree(label(t), [tree(x) for x in vals])
    else:
        copy_list = []
        for x in branches(t):
            copy_list.append(sprout_leaves(x, vals))
        return tree(label(t), copy_list)
        #copy = [sprout_leaves(x, vals) for x in branches(t)]
        #return tree(label(t), copy)
