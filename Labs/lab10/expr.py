...
class CallExpr(Expr):
    ...
    def eval(self, env):
        ...
        """
        "*** YOUR CODE HERE ***"
        #if self.operator in global_env:
        call_operator = self.operator.eval(env)
        call_operands = []
        for elem in self.operands:
            call_operands.append(elem.eval(env))

        return call_operator.apply(call_operands) #call_operator, self.operands[0].eval
