...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, beehive):
        ...
        place = self.place
        while place != None:
            if place.bees != [] and place.name != 'Hive':
                if self.spots_from_self >= self.min_range and self.spots_from_self <= self.max_range:
                    return random_or_none(place.bees)
            self.spots_from_self += 1
...
# END Problem 13
    ...
    def __init__(self, armor=1, true_queen=1):
        # BEGIN Problem 13
        "*** YOUR CODE HERE ***"
        self.armor = armor
        self.true_queen = not self.queen_exists
            ...
            while place.exit != None:
                ...
                if place.ant != None:
                    if place.ant.is_container:
                        if place.ant.contained_ant != None and place.ant.contained_ant.can_buff:
                            place.ant.contained_ant.damage *= 2
                            place.ant.contained_ant.can_buff = False
                    if place.ant.can_buff:
                        place.ant.damage *= 2
                        place.ant.can_buff = False
