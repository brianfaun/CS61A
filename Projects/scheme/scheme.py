...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        operands = rest.map(lambda nested_expr: scheme_eval(nested_expr, env))
...
class Frame(object):
    ...
    def make_child_frame(self, formals, vals):
        ...
        if formals.__len__() != vals.__len__():
            raise SchemeError('too many or too few vals given')
        child = Frame(self)
        while formals.__len__() > 0:
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        env.bindings[target] = scheme_eval(value, env)
        return target
        # END PROBLEM 5
...
def do_and_form(expressions, env):
    ...
    if len(expressions) == 0:
        return True
    holder = scheme_eval(expressions.first, env)
    if not scheme_truep(holder):
        return False
...
def do_define_macro(expressions, env):
    ...
    if isinstance(target, Pair) and scheme_symbolp(target.first):
        symbol, formals = target.first, target.rest
...
def optimize_tail_calls(original_scheme_eval):
    ...
    def optimized_eval(expr, env, tail=False):
        ...
        while isinstance(result, Thunk):
            optimized = original_scheme_eval(result.expr, result.env)
        return optimized
        # END
